...
class GrupIngrediente extends React.Component {
  ...
  render(){
    ...
    var mesajRautacios = "";
    if(this.totalCalorii < 200)
      mesajRautacios = mesaje[200];
    else if(this.totalCalorii < 500)
      mesajRautacios = mesaje[500];
    else if(this.totalCalorii < 1000)
      mesajRautacios = mesaje[1000];
    else if(this.totalCalorii < 2000)
      mesajRautacios = mesaje[2000];
    else
      mesajRautacios = mesaje["peste limita"];
    rez.push(React.createElement("p", null, mesajRautacios));
  return rez;
  }
}
...
window.addEventListener("load", function(){
  fetch("/Sources/json/ingrediente.json",
  {
    method:"GET",
